...
class handler(BaseHTTPRequestHandler):
   def do_GET(self):
       ...
       result = {
           ...
           "version": "4.7.0-SEMANTIC-MATCHING-FIXED",
   ...
   def _find_targeting_matches_progressive(self, user_message, targeting_data, original_message):
       ...
       home_improvement_keywords = [
           "floor", "floors", "flooring", "hardwood", "carpet", "tile", 
           "renovation", "remodel", "home improvement", "house", "property",
       ...
       if is_more_request or request_number > 1:
           ...
           has_fitness_intent = any(
               keyword in original_intent for keyword in fitness_keywords
           )
           has_home_improvement_intent = any(
               keyword in original_intent for keyword in home_improvement_keywords
           )
           print(f"🔄 SUBSEQUENT REQUEST #{request_number} - Using original intent: '{original_intent}'")
